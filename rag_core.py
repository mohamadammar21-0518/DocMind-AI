...
def build_vectorstore(chunks, collection_name="pdf_collection"):
    ...
    # Delete old vector store — must reset the chroma client first to release file locks
    if os.path.exists(CHROMA_DIR):
        ...
        try:
            ...
            print("[RAG] Old vector store cleared")
        except Exception as e:
            print(f"[RAG] Could not delete old store: {e} — continuing anyway")
...
def rerank_documents(query, docs, top_k=4):
    """
    Use cross-encoder to rerank retrieved documents by true relevance.
    ...
    print(f"[RAG] Reranked {len(docs)} -> top {len(top_docs)} chunks")
    return top_docs
